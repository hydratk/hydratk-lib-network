...
def get_dependencies():
    ...
    if (major == 2):
        dep_modules['jsonrpclib'] = {
            'min-version': '0.1.7',  'package': 'jsonrpclib'}
        dep_modules['MySQLdb'] = {
            'min-version': '1.2.3',  'package': 'MySQL-python',        'optional': True}
        dep_modules['ldap'] = {
            'min-version': '2.4.25', 'package': 'python-ldap'}
        ...
        dep_modules['jsonrpclib'] = {
            'min-version': '0.2.8',  'package': 'jsonrpclib-pelix'}
        dep_modules['MySQLdb'] = {
            'min-version': '1.3.7',  'package': 'mysqlclient',         'optional': True}
        dep_modules['ldap'] = {'min-version': '2.4.25', 'package': 'pyldap'}
        dep_modules['scapy'] = {
            'min-version': '0.18',   'package': 'scapy-python3'}
        dep_modules['stompest'] = {
            'min-version': '2.2.5',  'package': 'stompest',            'optional': True}
        dep_modules['suds'] = {'min-version': '1.3.2.0', 'package': 'suds-py3'}

    if (python_implementation() != 'PyPy'):
        dep_modules['psycopg2'] = {
            'min-version': '2.4.5',  'package': 'psycopg2',            'optional': True}
        dep_modules['pymssql'] = {
            'min-version': '2.1.3',  'package': 'pymssql',             'optional': True}
        if ('ORACLE_HOME' in environ):
            dep_modules['cx_Oracle'] = {
                'min-version': '5.1.3',  'package': 'cx_Oracle',           'optional': True}
        if ('JAVA_HOME' in environ):
            dep_modules['jpype'] = {
                'min-version': '0.6.1',  'package': 'JPype1',              'optional': True}
    else:
        dep_modules['psycopg2cffi'] = {
            'min-version': '2.7.4',  'package': 'psycopg2cffi',        'optional': True}
        dep_modules['pymssql'] = {
            'min-version': '2.1.2',  'package': 'pymssql',             'optional': True}
        if ('ORACLE_HOME' in environ):
            dep_modules['cx_Oracle'] = {
                'min-version': '0.1',    'package': 'cx-oracle-on-ctypes', 'optional': True}
