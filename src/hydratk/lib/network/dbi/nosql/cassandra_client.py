...
class DBClient(object):
    ...
    def connect(self, host, port=9042, key_space=None, user=None, passw=None, timeout=10):            
        ...
        try:
            ...
            message = '{0}/{1}@{2}:{3}/{4} timeout:{5}'.format(user, passw, host, port, key_space, timeout)
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg('htk_dbi_connecting', message), self._mh.fromhere())
            
            ev = event.Event('dbi_before_connect', host, port, key_space, user, passw, timeout)
            ...
            if (ev.will_run_default()):
                ...
                points = [self._host] if (self._host.__class__.__name__ != 'list') else self._host
                auth = PlainTextAuthProvider(self._user, self._passw) if (self._user != None) else None
                self._client = Cluster(contact_points=points, port=self._port, auth_provider=auth, control_connection_timeout=timeout) 
