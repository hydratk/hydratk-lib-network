...
class DBClient(object):
    ...
    def connect(self, host=None, port=1521, sid=None, user=None, passw=None):
        ...
        try:
            ...
            ev = event.Event(
                'dbi_before_connect', host, port, sid, user, passw)
            ...
            if (ev.will_run_default()):
                ...
                dsn_str = '{0}:{1}/{2}'.format(self._host,
                                               self._port, self._sid)
                self._client = connect(
                    user=self._user, password=self._passw, dsn=dsn_str)
            ...
            if (ev.will_run_default()):
                ...
                if (bindings != None):
                    for i in range(0, len(bindings)):
                        query = replace(query, '?', ':{0}'.format(
                            i + 1), 1) if (version_info[0] == 2) else query.replace('?', ':{0}'.format(i + 1), 1)
    ...
    def call_proc(self, p_name, param_names=[], i_values={}, o_types={}, type='proc', ret_type=None, autocommit=True):
        ...
        try:
            ...
            ev = event.Event('dbi_before_call_proc', p_name,
                             param_names, i_values, o_types, type, ret_type, autocommit)
            if (self._mh.fire_event(ev) > 0):
                p_name = ev.argv(0)
            ...
            if (ev.will_run_default()):
                if (type not in ('func', 'function', 'proc', 'procedure')):
                    self._mh.dmsg('htk_on_error', self._mh._trn.msg(
                        'htk_dbi_unknown_type', self.type), self._mh.fromhere())
                    return None
                ...
                for name in param_names:
                    if (name in i_values):
                        ...
                        if (o_types[name].lower() == 'cursor'):
                            ...
                            params.append(
                                cur.var(self._data_types[o_types[name].lower()]))
                    else:
                        params.append(None)

                if (type in ('func', 'function')):
                    result = cur.callfunc(
                        p_name, self._data_types[ret_type.lower()], params)
                else:
            ...
            cur.close()
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg(
                'htk_dbi_proc_called', output), self._mh.fromhere())
