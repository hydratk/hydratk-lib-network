...
from ldap import set_option, initialize, LDAPError, OPT_DEBUG_LEVEL, SCOPE_BASE, SCOPE_SUBTREE
...
class LDAPClient:
    ...
    def connect(self, host, base_dn, port=None, secured=False, user=None, passw=None):
        ...
        try:            
            ...
            message = 'host:{0}, base_dn:{1}, port:{2}, secured:{3}, user:{4}, passw:{5}'.format(
                       host, base_dn, port, secured, user, passw)                            
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg('htk_ldap_connecting', message), self._mh.fromhere())

            ev = event.Event('ldap_before_connect', host, base_dn, port, secured, user, passw)
                ...
                if (cn_only):
                    ...
                    for rec in recs:
                        ...
                        for key, value in record.items():
                            if (value.__class__.__name__ == 'list' and len(value) == 1):
                                record[key] = value[0]
